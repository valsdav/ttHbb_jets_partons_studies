...
@njit(TResult(TPrediction, TInt64[::1], TInt64))
def extract_prediction_nomasking(predictions, num_partons, max_jets):
    float_negative_inf = -np.float32(np.inf)
    max_partons = num_partons.max()
    num_targets = len(predictions)
    ...
    for i in range(num_targets):
        ...
        best_jet = np.argmax(predictions[i])
        best_value = predictions[i][best_jet]
        
        #best_jet, best_prediction, best_value = maximal_prediction(predictions)
        
        # print("------------------")
        # print("best jets", best_jet)
        # print("best target", best_prediction)
        # print("best value", best_value)
        
        if not np.isfinite(best_value):
            continue

        best_jets = unravel_index(best_jet, strides[i])
        # print("best jet unravel", best_jets)

        results[i, :] = -1
        for j in range(num_partons[i]):
            results[i, j] = best_jets[j]

        # predictions[best_prediction][:] = float_negative_inf
        # # print("num_partons", num_partons)
        # for i in range(num_targets):
        #     for jet in best_jets:
        #         mask_jet(predictions[i], num_partons[i], max_jets, jet, float_negative_inf)
...
@njit(TResults(TPredictions, TInt64[::1], TInt64, TInt64), parallel=True)
def _extract_predictions_nomasking(predictions, num_partons, max_jets, batch_size):
    output = np.zeros((batch_size, len(predictions), num_partons.max()), np.int64)
    predictions = [p.copy() for p in predictions]

    for batch in numba.prange(batch_size):
        current_prediction = numba.typed.List([prediction[batch] for prediction in predictions])
        output[batch, :, :] = extract_prediction_nomasking(current_prediction, num_partons, max_jets)

    return np.ascontiguousarray(output.transpose((1, 0, 2)))

def extract_predictions(predictions: List[TArray], masking=True):
    ...
    if masking:
        ...
        results = _extract_predictions_nomasking(flat_predictions, num_partons, max_jets, batch_size)
